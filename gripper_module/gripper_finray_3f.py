...
class GripperFinray3F(GripperBase):
    def __init__(self, bullet_client, gripper_size, gripper_height=None):
        ...
        _gripper_height = gripper_height if gripper_height is not None else 0.10
        print("Finray 3f _gripper_height is : ", _gripper_height)
    ...
    def load(self, basePosition):
        gripper_urdf = "assets/gripper/finray3f/urdf/finray3f.urdf"
    ...
    def get_vis_pts(self, open_scale):
        ...
        return np.array([
            [0, open_length],
            [open_length * np.cos(np.pi/6), -open_length * np.cos(np.pi/3)],
            [-open_length * np.cos(np.pi/6), -open_length * np.cos(np.pi/3)],
        ])